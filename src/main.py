...
def create_club(club: Club):
    ...
    try:
        # This is scenario to throw an error in code when you provide 0 as
        # established date
...
def delete_club(club_name: str):
    ...
    try:
        ...
        # log exception when there's an error
        error_msg = "An error occured while deleting club id"
        logger.exception(f'{error_msg}:{club_index}, {club_name}')
        return f'{error_msg}:{club_index}, {club_name}'
...
async def log_custom_metric():
    ...
    loop_view = view_module.View(
        "metric name: club stats",
        "number of loop",
        [],
        loop_measure,
